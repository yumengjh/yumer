...
> 覆盖两阶段工作：基础设施加固（Phase 1）与 digest / 恢复链路补强（Phase 2）  
> 涉及仓库：`yuediter`（前端）、`yumer-server`（后端）
...
工作分两个阶段推进，均属于**同步基础设施加固**——用户界面无可见变化，但刷新后内容与顺序的稳定性显著提升。
...
| 视觉序 vs canonical 序 | 编辑器显示与 DB 序短暂不一致 | ⚠️ 靠 move 队列收敛 |
...
| 范围 | Phase 1 | Phase 2 |
|------|---------|---------|
| 前端 `src/services/sync` 单测 | 99/99 | 101/101 |
| 前端 source guards | — | 11/11 |
| 后端 `blocks-sync-idempotency` | 19/19 | 19/19 |
| 用户复测（高频编辑 + 刷新） | 正常 | 待 Phase 2 部署后复测 |

新增用例：delete ACK 快照移除、ACK 前快照补丁、digest sortKey 敏感性、`adoptServerDraftRevision`、draft-revision resync / idle repair 的 source guard。
...
| 项 | 说明 | 建议实现 |
|----|------|----------|
| 同步健康仪表盘 | 聚合 trace / 日志指标 | `DRAFT_REVISION_MISMATCH` 率、`digestMatch` 跳过率、`orphaned-create` 触发率、`identity:resurrected` 计数 |
| 端到端场景自动化 | 覆盖分析文档中的不稳定场景 | Playwright：粘贴 200 段 → 同步中全删再贴 → 等待 idle → 刷新断言；弱网 + 关标签重开 |
| digest 跨端契约测试 | 防止 FE/BE 算法再次分叉 | 共享 fixture JSON（块列表 + 期望 digest），两端各跑单测 |

**完成标准**：任一典型文档 ID 可在 5 分钟内从日志还原 block 生命周期；CI 覆盖「大批量 + 快速交替编辑」至少 3 条路径。
...
| 视觉序 digest（仅客户端） | 在 idle 自检中比较 `content[]` 视觉 blockId 序与 canonical sortKey 序；不一致则强制 enqueue move（不发给服务端比对） |
| 操作依赖图 batching | create 与其后的 move/update 优先同批，减少中间态被 reconcile 看到 |
...
在此 invariant 下，高频单 tab 编辑 + 刷新应与编辑结束前一致。剩余主要风险在**多标签页并发**与**冲突态用户体验**；下一步按 §7 的 P0→P1→P2 顺序推进，优先可观测性与 E2E 回归，再做多 tab 与协议精度。
