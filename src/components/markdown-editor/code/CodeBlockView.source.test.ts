...
describe("CodeBlockView source contract", () => {
  ...
  it("uses a focused centered tab for status bar collapse and restore", () => {
    ...
    expect(source).toContain('setAttr("statusBarCollapsed", true)');
    expect(source).toContain('setAttr("statusBarCollapsed", false)');
  ...
  it("shows the status tab only while the Tiptap selection is inside the code block", () => {
    ...
    expect(css).toContain(".code-block-view.is-status-collapsed .code-block-status-restore-tab");
